...
var INITIAL_DOCUMENT_OUTDATED_TRACE_ID_THRESHOLD = 2 * _browserCore.ONE_MINUTE;
...
function getDocumentTraceDataFromComment(document) {
  ...
  return createDocumentTraceData((0, _browserCore.findCommaSeparatedValue)(comment, 'trace-id'), (0, _browserCore.findCommaSeparatedValue)(comment, 'trace-time'));
}
...
function findTraceComment(document) {
  ...
  if (document.body) {
    for (var i = document.body.childNodes.length - 1; i >= 0; i -= 1) {
      ...
      if (!(0, _browserCore.isTextNode)(node)) {
        break;
      }
    }
  }
...
function getTraceCommentFromNode(node) {
  if (node && (0, _browserCore.isCommentNode)(node)) {
