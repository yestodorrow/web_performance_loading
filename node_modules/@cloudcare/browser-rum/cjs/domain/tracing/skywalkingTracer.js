"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.SkyWalkingTracer = SkyWalkingTracer;
var _browserCore = require("@cloudcare/browser-core");
// start SkyWalking
function uuid() {
  return 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, function (c) {
    /* tslint:disable */
    var r = Math.random() * 16 | 0;
    /* tslint:disable */
    var v = c === 'x' ? r : r & 0x3 | 0x8;
    return v.toString(16);
  });
}
/**
 *
 * @param {*} traceId
 * @param {*} traceSegmentId
 * @param {*} applicationId
 * @param {*} version
 * @param {*} requestUrlHost
 */
function getSkyWalkingSw8(traceId, traceSegmentId, applicationId, env, version, requestUrlHost) {
  try {
    var traceIdStr = String((0, _browserCore.base64Encode)(traceId));
    var segmentId = String((0, _browserCore.base64Encode)(traceSegmentId));
    var service = String((0, _browserCore.base64Encode)(applicationId + '_rum_' + env));
    var instance = String((0, _browserCore.base64Encode)(version));
    var endpoint = String((0, _browserCore.base64Encode)(window.location.href));
    var peer = String((0, _browserCore.base64Encode)(requestUrlHost));
    var index = '0';
    // var values = `${1}-${traceIdStr}-${segmentId}-${index}-${service}-${instance}-${endpoint}-${peer}`;
    return '1-' + traceIdStr + '-' + segmentId + '-' + index + '-' + service + '-' + instance + '-' + endpoint + '-' + peer;
  } catch (err) {
    return '';
  }
}
/**
 *
 * @param {*} configuration  配置信息
 * @param {*} requestUrl 请求的url
 */
function SkyWalkingTracer(configuration, requestUrl, traceSampled) {
  this._spanId = uuid();
  this._traceId = uuid();
  this._applicationId = configuration.applicationId;
  this._env = configuration.env;
  this._version = configuration.version;
  this._urlParse = (0, _browserCore.urlParse)(requestUrl).getParse();
  this._traceSampled = traceSampled;
}
SkyWalkingTracer.prototype = {
  isTracingSupported: function isTracingSupported() {
    if (this._env && this._version && this._urlParse) return true;
    return false;
  },
  getSpanId: function getSpanId() {
    return this._spanId;
  },
  getTraceId: function getTraceId() {
    return this._traceId;
  },
  getSkyWalkingSw8: function getSkyWalkingSw8() {
    try {
      var traceIdStr = String((0, _browserCore.base64Encode)(this._traceId));
      var segmentId = String((0, _browserCore.base64Encode)(this._spanId));
      var service = String((0, _browserCore.base64Encode)(this._applicationId + '_rum_' + this.env));
      var instance = String((0, _browserCore.base64Encode)(this._version));
      var endpoint = String((0, _browserCore.base64Encode)(window.location.href));
      var peer = String((0, _browserCore.base64Encode)(this._urlParse.Host));
      var index = '0';
      // var values = `${1}-${traceIdStr}-${segmentId}-${index}-${service}-${instance}-${endpoint}-${peer}`;
      return (this._traceSampled ? '1' : '0') + '-' + traceIdStr + '-' + segmentId + '-' + index + '-' + service + '-' + instance + '-' + endpoint + '-' + peer;
    } catch (err) {
      return '';
    }
  },
  makeTracingHeaders: function makeTracingHeaders() {
    return {
      sw8: this.getSkyWalkingSw8()
    };
  }
};
//# sourceMappingURL=skywalkingTracer.js.map