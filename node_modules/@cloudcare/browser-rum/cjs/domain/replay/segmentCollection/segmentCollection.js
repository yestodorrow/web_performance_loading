...
exports.SEGMENT_DURATION_LIMIT = exports.SEGMENT_BYTES_LIMIT = void 0;
...
var _buildReplayPayload = require("./buildReplayPayload");
var _segment = require("./segment");
var SEGMENT_DURATION_LIMIT = 30 * _browserCore.ONE_SECOND;
/**
 * beacon payload max queue size implementation is 64kb
 * ensure that we leave room for logs, rum and potential other users
 */
exports.SEGMENT_DURATION_LIMIT = SEGMENT_DURATION_LIMIT;
...
exports.SEGMENT_BYTES_LIMIT = SEGMENT_BYTES_LIMIT;
...
function doStartSegmentCollection(lifeCycle, getSegmentContext, httpRequest, encoder) {
  ...
  var subscribeViewCreated = lifeCycle.subscribe(_browserCore.LifeCycleEventType.VIEW_CREATED, function () {
    flushSegment('view_change');
  });
  var unsubscribeViewCreated = subscribeViewCreated.unsubscribe;
  var subscribePageExited = lifeCycle.subscribe(_browserCore.LifeCycleEventType.PAGE_EXITED, function (pageExitEvent) {
  ...
  function flushSegment(flushReason) {
    if (state.status === SegmentCollectionStatus.SegmentPending) {
      state.segment.flush(function (metadata) {
        var payload = (0, _buildReplayPayload.buildReplayPayload)(encoder.getEncodedBytes(), metadata, encoder.getRawBytesCount());
        if ((0, _browserCore.isPageExitReason)(flushReason)) {
          httpRequest.sendOnExit(payload);
        } else {
          httpRequest.send(payload);
        }
      });
      (0, _browserCore.clearTimeout)(state.expirationTimeoutId);
    }
    if (flushReason !== 'stop') {
      state = {
        status: SegmentCollectionStatus.WaitingForInitialRecord,
    ...
    addRecord: function addRecord(record) {
      ...
      if (state.status === SegmentCollectionStatus.WaitingForInitialRecord) {
        ...
        state = {
          ...
          segment: new _segment.Segment(encoder, context, state.nextSegmentCreationReason),
          expirationTimeoutId: (0, _browserCore.setTimeout)(function () {
...
function setSegmentBytesLimit() {
  var newSegmentBytesLimit = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 60000;
  exports.SEGMENT_BYTES_LIMIT = SEGMENT_BYTES_LIMIT = newSegmentBytesLimit;
}
