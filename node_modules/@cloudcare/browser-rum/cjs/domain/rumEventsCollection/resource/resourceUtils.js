...
exports.isResourceUrlLimit = isResourceUrlLimit;
...
exports.FAKE_INITIAL_DOCUMENT = FAKE_INITIAL_DOCUMENT;
var RESOURCE_TYPES = [[_browserCore.ResourceType.DOCUMENT, function (initiatorType) {
  return FAKE_INITIAL_DOCUMENT === initiatorType;
}], [_browserCore.ResourceType.XHR, function (initiatorType) {
  return 'xmlhttprequest' === initiatorType;
}], [_browserCore.ResourceType.FETCH, function (initiatorType) {
  return 'fetch' === initiatorType;
}], [_browserCore.ResourceType.BEACON, function (initiatorType) {
  return 'beacon' === initiatorType;
}], [_browserCore.ResourceType.CSS, function (_, path) {
  return path.match(/\.css$/i) !== null;
}], [_browserCore.ResourceType.JS, function (_, path) {
  return path.match(/\.js$/i) !== null;
}], [_browserCore.ResourceType.IMAGE, function (initiatorType, path) {
  return (0, _browserCore.includes)(['image', 'img', 'icon'], initiatorType) || path.match(/\.(gif|jpg|jpeg|tiff|png|svg|ico)$/i) !== null;
}], [_browserCore.ResourceType.FONT, function (_, path) {
  return path.match(/\.(woff|eot|woff2|ttf)$/i) !== null;
}], [_browserCore.ResourceType.MEDIA, function (initiatorType, path) {
  return (0, _browserCore.includes)(['audio', 'video'], initiatorType) || path.match(/\.(mp3|mp4)$/i) !== null;
}]];
function computeResourceKind(timing) {
  var url = timing.name;
  if (!(0, _browserCore.isValidUrl)(url)) {
    return _browserCore.ResourceType.OTHER;
  }
  var path = (0, _browserCore.getPathName)(url);
  var type = _browserCore.ResourceType.OTHER;
  (0, _browserCore.each)(RESOURCE_TYPES, function (res) {
    var _type = res[0],
      isType = res[1];
...
function areInOrder() {
  var numbers = (0, _browserCore.toArray)(arguments);
...
function computePerformanceResourceDuration(entry) {
  ...
  if (entry.duration === 0 && entry.startTime < entry.responseEnd) {
    return (0, _browserCore.msToNs)(entry.responseEnd - entry.startTime);
  }
  return (0, _browserCore.msToNs)(entry.duration);
}
...
function computePerformanceResourceDetails(entry) {
  ...
  var details = {
    firstbyte: (0, _browserCore.msToNs)(responseStart - domainLookupStart),
    trans: (0, _browserCore.msToNs)(responseEnd - responseStart),
  ...
  if (responseStart > 0 && responseStart <= (0, _browserCore.preferredNow)()) {
    details.ttfb = (0, _browserCore.msToNs)(responseStart - requestStart);
  }
  // Make sure a connection occurred
  if (connectEnd !== fetchStart) {
    details.tcp = (0, _browserCore.msToNs)(connectEnd - connectStart);
    details.connectTime = formatTiming(startTime, connectStart, connectEnd);
    // Make sure a secure connection occurred
    if (areInOrder(connectStart, secureConnectionStart, connectEnd)) {
      details.ssl = (0, _browserCore.msToNs)(connectEnd - secureConnectionStart);
  ...
  if (domainLookupEnd !== fetchStart) {
    details.dns = (0, _browserCore.msToNs)(domainLookupEnd - domainLookupStart);
    details.dnsTime = formatTiming(startTime, domainLookupStart, domainLookupEnd);
  }
  if (hasRedirection(entry)) {
    details.redirect = (0, _browserCore.msToNs)(redirectEnd - redirectStart);
...
function toValidEntry(entry) {
  ...
  return (0, _browserCore.extend)({}, entry, {
...
function formatTiming(origin, start, end) {
  return {
    duration: (0, _browserCore.msToNs)(end - start),
    start: (0, _browserCore.msToNs)(start - origin)
  };
}
...
function isAllowedRequestUrl(configuration, url) {
  return url && !(0, _browserCore.isIntakeRequest)(url, configuration);
}
